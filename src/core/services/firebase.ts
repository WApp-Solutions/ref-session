...
export const useFirebaseFirestoreReader = <T = DocumentData>(
    path: string
): T[] => {
    const pathSegments = path.split('/')
    ...
    const [cleanData, setCleanData] = useState<T[]>([])

    useEffect(() => {
        const doGetDocs = async () => {
            console.log('select from DB')
            const docs = await getDocs(
                query<T>(
                    collection(db, pathToLastCollectionInPath).withConverter(
                        converter<T>()
                    )
                )
            )
...
export const useFirebaseFirestoreWriter = <T = DocumentData>(path: string) => {
    const pathSegments = path.split('/')
